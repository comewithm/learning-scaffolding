...
export default class Creator {
    ...
    async downloadPlugins() {
        ...
        this.fileModule.packageJSON.setPkg(await fs.readJSON(pkgPath))
        const pluginDeps = Object.keys(this.pluginModule.selectedPlugins)
        if(pluginDeps.length) {
            pluginDeps.forEach((dep) => {
                this.fileModule.packageJSON.pkg.devDependencies[dep] = 'latest'
            })
            // tool
    ...
    async installDependencies() {
        const outputConfig = {cwd: this.projectDir, studio: 'inherit'}
        const command = this.tool === 'npm' ? 'install' : ''
        await wrapLoading(execa, `项目依赖安装ing...`, this.tool, [command], outputConfig)
    }
    ...
    async create() {
        ...
        if(hasDownloadPlugins) {
            ...
            Object.keys(this.pluginModule.selectedPlugins).forEach(dep => {
                delete this.fileModule.packageJSON.pkg.devDependencies[dep]
            })
        }

        // 覆盖package.json
        this.fileModule.files['package.json'] = JSON.stringify(this.fileModule.packageJSON.pkg, null, 2)
        // 5.写入文件到项目项目中
