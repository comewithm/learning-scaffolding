...
export default class Creator {
    ...
    async downloadPlugins() {
        ...
        if(pluginDeps.length) {
            pluginDeps.forEach((dep) => {
                this.pkg.devDependencies[dep] = 'latest'
            })
            // tool
            const command = this.tool === 'npm' ? 'install' : 'add'
            const outputConfig = {cwd: this.projectDir, studio: 'inherit'}
            await wrapLoading(execa, `${pluginDeps.join(', ')}安装...`, this.tool, [command, ...pluginDeps, '-D'], outputConfig)
        }
    }
    ...
    async create() {
        ...
        if(hasDownloadPlugins) {
            ...
            // 4.2插件执行 => 转换的文件都在FileInfo中
            await this.pluginModule.applyPlugins(this.fileModule)
            // 4.3执行文件转换
            await this.fileModule.renderFiles()
            // 4.4package.json中删除插件的依赖
            Object.keys(this.pluginModule.selectedPlugins).forEach(dep => {
                delete this.pkg.devDependencies[dep]
            })
        }

        // 覆盖package.json
        this.fileModule.files['package.json'] = JSON.stringify(this.pkg, null, 2)
        // 5.写入文件到模版项目中
